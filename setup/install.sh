...
if grep -q "/home/" $(pwd) | grep "setup/"; then
    ...
    read
    echo -e "$plus Copying files to /opt/scoring_engine..."
    mkdir -p /opt/minos
    cp -rf ../ /opt/minos
else
	echo -e "$err Install does not know if it can use relative copy safely. Please make sure you did not move install.sh from its original location in the repository."
	exit 1
fi
...
echo -e "$plus Installing web software..."
    ...
    pip3 install Flask flask-login flask-wtf bcrypt uwsgi timeout_decorator python-ldap
